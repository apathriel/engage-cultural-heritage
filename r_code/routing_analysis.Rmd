---
title: "routing_analysis"
output: html_document
date: "2024-06-03"
---

```{r setup, include=FALSE}
library(knitr)
library(rmdformats)

options(max.print="75")
opts_chunk$set(echo=TRUE,
	             cache=TRUE,
               prompt=FALSE,
               tidy=TRUE,
               comment=NA,
               message=FALSE,
               warning=FALSE)
opts_knit$set(width=75)
```


```{r packages}
install.packages("Rcpp")
install.packages("fasterize")
install.packages("mapboxapi", dependencies = TRUE)
install.packages("tidyverse")
install.packages("readr")
install.packages("sf")
```

```{r mapbox-auth, eval = FALSE}
api_token <- readr::read_file("mytoken.txt")

library(mapboxapi)
mb_access_token(api_token, install = TRUE, overwrite = TRUE)
readRenviron("~/.Renviron")
```


```{r geocode}
dk <- mb_geocode("Denmark")
dk
```

```{r load_data, eval=FALSE}
library(knitr)
library(rmdformats)
library(tidyverse)
library(sf)
library(dplyr)

monuments <- read_sf("data/anlaeg_all_25832.shp")

monuments_crs <- st_as_sf(monuments, coords = "geometry")
monuments_crs <- st_transform(monuments_crs, crs = 4326)
monuments_crs

test_crs <- monuments_crs %>% 
  filter(anlaegsbet == "Rundhøj")

test_crs %>% 
  slice(1:10000)

```
```{r init_colors}

```

```{r mabox-map}
library(leaflet)
library(mapboxapi)

mapbox_map <- leaflet() %>%
  addMapboxTiles(style_id = "streets-v11",
                 username = "mapbox") %>% 
  setView(lng = dk[1],
          lat = dk[2], 
          zoom = 6.5)
  
mapbox_map %>%
  addCircleMarkers(data = test_crs,
                    popup = test_crs$anlaegsbet,
                    radius = 5,
                    weight = 1,
                   clusterOptions = markerClusterOptions(showCoverageOnHover = TRUE))
```