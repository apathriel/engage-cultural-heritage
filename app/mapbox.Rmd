...
initial_position <- data.frame(place="Initial position", longitude=10.2033485, latitude=56.174346)
initial_position <- mb_geocode("Vesterbro Torv, Aarhus")

initial_sf <- st_as_sf(initial_position, coords = c("longitude", "latitude"), crs = 4326)

coords <- st_coordinates(initial_sf)
coords
...
# Identify monuments that intersect with the buffer
monuments_int <- st_intersects(nb_buf, monument_attraction)

# Get the indexes of the monuments within buffer by extracting first element of each list
int <- monuments_int[[1]]

# Get the names of monuments within the buffer
monument_attraction$stednavn[int]

# Clip monuments by buffer
...
map %>%
  addPolygons(data=nb_buf$geometry) %>% 
  addCircleMarkers(data = monument_attraction_clip,
                   ...
                   fillOpacity = 0.8,
                   color = "red") %>% 
  addCircleMarkers(data = initial_sf,
                 radius = 3,
                 stroke = FALSE,
                 fillOpacity = 0.8,
                 color = "blue")
...
for (layer_data in data_split) {
  ...
  mapbox_map <- mapbox_map %>%
    addCircleMarkers(data = layer_data,
                     group = group_name,
                     popup = paste(sep = "<br/>",
                                   "<b><a href=''>Et test-link</a></b>",
                                   layer_data$anlaegsbet,
                                   paste0("Her kan man skrive, hvad en ", layer_data$anlaegsbet, " er")),
                     label = ~stednavn,
                     clusterOptions = markerClusterOptions(),
                     radius = 8,
                     stroke = FALSE,
